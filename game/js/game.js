...
(function () {
  ...
  function startLevel(i) {
    ...
    Game.cam.x = clamp(st.x - 260, 0, Math.max(0, Game.level.data.width - VIEW_W));
    Game.cam.y = 60;
  ...
  function updateCamera(dt) {
    var p = player;
    var tx = p.x + p.w / 2 - VIEW_W * 0.42 + p.vx * 0.12;
    var ty = p.y + p.h / 2 - VIEW_H * 0.55;
    var kx = Math.min(1, 8 * dt), ky = Math.min(1, 6 * dt);
    Game.cam.x += (tx - Game.cam.x) * kx;
    Game.cam.y += (ty - Game.cam.y) * ky;
    Game.cam.x = clamp(Game.cam.x, 0, Math.max(0, Game.level.data.width - VIEW_W));
    Game.cam.y = clamp(Game.cam.y, -180, 60);
  }
  ...
  function drawParallax() {
    ...
    for (var i = 0; i < d.stars.length; i++) {
      ...
      x = ((x % (Game.level.data.width + VIEW_W * 2)) + Game.level.data.width + VIEW_W * 2) % (Game.level.data.width + VIEW_W * 2) - VIEW_W;
    ...
    for (i = 0; i < d.dust.length; i++) {
      ...
      px = ((px % (Game.level.data.width + VIEW_W * 2)) + Game.level.data.width + VIEW_W * 2) % (Game.level.data.width + VIEW_W * 2) - VIEW_W;
      var py = p.y + Math.sin(t * p.s + p.ph) * 14;
  ...
  function drawShapes(arr, factor, color) {
    ...
    var total = L.width + VIEW_W * 2;
    ctx.fillStyle = color;
    for (var i = 0; i < arr.length; i++) {
      var s = arr[i];
      var x = s.x - Game.cam.x * factor;
      x = ((x % total) + total) % total - VIEW_W;
      ctx.beginPath();
      ctx.moveTo(x - s.w / 2, VIEW_H);
      ctx.lineTo(x - s.w / 2 + s.r, s.y);
      ctx.lineTo(x + s.w / 2 - s.r, s.y);
      ctx.lineTo(x + s.w / 2, VIEW_H);
      ctx.closePath();
      ctx.fill();
    }
  }
  ...
  function drawScreenEffects() {
    ...
    if (darkA > 0) {
      var g = ctx.createRadialGradient(VIEW_W / 2, VIEW_H / 2, VIEW_H * 0.4, VIEW_W / 2, VIEW_H / 2, VIEW_H * 0.85);
  ...
  function drawHUD() {
    ...
    ctx.fillText('● ' + Game.coins + ' / ' + L.data.coinTotal, VIEW_W - 18, 34);
    ctx.fillStyle = 'rgba(255,255,255,0.9)';
    ctx.fillText(Game.time.toFixed(1) + 's', VIEW_W - 18, 56);
    ...
    ctx.save();
    roundRect(ctx, 16, VIEW_H - 58, 190, 40, 20);
    ...
    ctx.beginPath(); ctx.arc(44, VIEW_H - 38, 9, 0, 6.2832); ctx.fill();
    ctx.fillStyle = 'rgba(255,255,255,0.95)';
    ctx.font = 'bold 15px "Segoe UI","Microsoft YaHei",sans-serif';
    ctx.textAlign = 'left';
    ctx.fillText(label, 62, VIEW_H - 33);
    ctx.font = '11px "Segoe UI","Microsoft YaHei",sans-serif';
    ctx.fillStyle = 'rgba(255,255,255,0.55)';
    ctx.fillText('Q / E 切换', 62, VIEW_H - 18);
    ctx.restore();
    ...
    if (!Game.isTouch) {
      ...
      ctx.fillText('R 重开 · M 静音 · P 暂停', VIEW_W - 18, VIEW_H - 22);
    }
    ...
    if (Game.hintT > 0 && Game.state === 'playing') {
      ...
      var hw = Math.min(VIEW_W - 60, 780);
      wrapText(ctx, L.data.hint, VIEW_W / 2, 96, hw, 22);
    }
  }
